...
divHeading.append(h1);
document.body.append(divHeading);
...
function addBook(obj) {
  ...
  imageBook.setAttribute('src', `${obj['image']}`);
  divCard.append(imageBook);

  let bookTitle = document.createElement("h2");
  bookTitle.innerText = obj['title'];
  divCard.append(bookTitle);
  ...
  divCard.append(buyButton); 

  return divCard;
}

for (var elem of data['books']) {
    let divContainer = document.querySelector('div.container');
    divContainer.append(addBook(elem));
}
